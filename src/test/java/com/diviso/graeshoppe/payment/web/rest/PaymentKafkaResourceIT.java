...
class PaymentKafkaResourceIT {
    ...
    void setup() {
        ...
        PaymentKafkaResource kafkaResource = new PaymentKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/payment-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/payment-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
